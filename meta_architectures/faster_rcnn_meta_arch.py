...
def visualize_box_list(image, boxlist, class_list, normalized=True):
  ...
  if not isinstance(image, tf.Tensor):
      image = tf.convert_to_tensor(np.zeros((1000, 1000, 3), np.uint8))
  test_num = 5
  with tf.Session(config=config) as sess:
    ...
    for test_idx in range(test_num):
      result = sess.run([image,
          boxlist.get_field('boxes'),
          boxlist.get_field('oriented_boxes'),
          class_list])
      img = result[0].astype(np.uint8)
      img2 = np.copy(img)
      colors = [(0, 0, 255), (0, 255, 0), (255, 0, 0), (255, 255, 0)]
      for i in range(0, result[1].shape[0]):
        box = result[1][i]
        oriented_box = result[2][i]
        if normalized:
          (minx, miny, maxx, maxy) = (box[1] * img.shape[1],
                box[0] * img.shape[0], box[3] * img.shape[1],
                box[2] * img.shape[0])
        else:
          minx, miny, maxx, maxy = box[1], box[0], box[3], box[2]
        cv2.rectangle(img, (int(minx), int(miny)),
                (int(maxx), int(maxy)), colors[i%len(colors)], 2)
        for j in range(0, oriented_box.shape[0]):
            if normalized:
              point1 = (int(oriented_box[j][1] * img.shape[1]),
                    int(oriented_box[j][0] * img.shape[0]))
              point2 = (int(oriented_box[(j+1)%4][1] * img.shape[1]),
                    int(oriented_box[(j+1)%4][0] * img.shape[0]))
            else:
              point1 = (int(oriented_box[j][1]),
                    int(oriented_box[j][0]))
              point2 = (int(oriented_box[(j+1)%4][1]),
                    int(oriented_box[(j+1)%4][0]))
            cv2.line(img2, point1, point2, colors[j%len(colors)], 2)
      outname = 'test/' + str(test_idx) + '_1.jpg'
      cv2.imwrite(outname, img)
      outname = 'test/' + str(test_idx) + '_2.jpg'
...
class FasterRCNNMetaArch(model.DetectionModel):
  ...
  def __init__(self,
               ...
               second_stage_localization_loss_weight_oriented,
    ...
    Args:
      ...
      second_stage_localization_loss_weight_oriented: A float
    ...
    self._proposal_target_assigner = target_assigner.create_target_assigner(
        reference='FasterRCNN',
        stage='proposal')
    self._detector_target_assigner = target_assigner.create_target_assigner(
        reference='R2CNN',
        stage='detection', unmatched_cls_target=unmatched_cls_target)
    # Both proposal and detector target assigners use the same box coder
    self._proposal_box_coder = self._proposal_target_assigner.box_coder
    self._detector_box_coder = self._detector_target_assigner.box_coder
    ...
    self._first_stage_box_predictor = box_predictor.ConvolutionalBoxPredictor(
        ...
        box_code_size=self._proposal_box_coder.code_size)
    ...
    self._second_stage_localization_loss = (
        losses.WeightedSmoothL1LocalizationLoss(anchorwise_output=True))
    self._second_stage_localization_loss_oriented = (
        losses.WeightedSmoothL1LocalizationLoss(anchorwise_output=True))
    self._second_stage_classification_loss = (
        losses.WeightedSoftmaxClassificationLoss(anchorwise_output=True))
    self._second_stage_loc_loss_weight = second_stage_localization_loss_weight
    self._second_stage_loc_loss_weight_oriented = second_stage_localization_loss_weight_oriented
    ...
    Returns:
      prediction_dict: a dictionary holding "raw" prediction tensors:
        ...
        4) rpn_box_encodings:  3-D float tensor of shape
          [batch_size, num_anchors, self._proposal_box_coder.code_size] containing
          predicted boxes.
  ...
  def _predict_second_stage(self, rpn_box_encodings,
    ...
    Args:
      rpn_box_encodings: 3-D float tensor of shape
        [batch_size, num_valid_anchors, self._proposal_box_coder.code_size] containing
        predicted boxes.
      ...
      anchors: 2-D float tensor of shape
        [num_anchors, self._proposal_box_coder.code_size].
      image_shape: A 1D int32 tensors of size [4] containing the image shape.

    Returns:
      prediction_dict: a dictionary holding "raw" prediction tensors:
        1) refined_box_encodings: a 3-D tensor with shape
          [total_num_proposals, num_classes, 5] representing predicted
          (final) oriented box encoding.
          total_num_proposals=batch_size*self._max_num_proposals
    ...
    if False:
      ...
      with tf.Session(config=config) as sess:
        sess.run(init_op)
        tf.train.start_queue_runners(sess)
        results = sess.run(box_predictions)
        import pdb
        pdb.set_trace()
        print('a')
    ...
    refined_box_encodings_oriented = tf.squeeze(
        box_predictions[box_predictor.BOX_ENCODINGS_ORIENTED], axis=1)
    ...
    prediction_dict = {
        ...
        'refined_box_encodings_oriented': refined_box_encodings_oriented,
  ...
  def postprocess(self, prediction_dict):
    ...
    with tf.name_scope('SecondStagePostprocessor'):
      ...
      detections_dict = self._postprocess_box_classifier(
          ...
          prediction_dict['refined_box_encodings_oriented'],
  ...
  def _postprocess_rpn(self,
    ...
    if False:
      ...
      with tf.Session(config=config) as sess:
        sess.run(init_op)
        tf.train.start_queue_runners(sess)
        result = sess.run([proposal_boxes, proposal_scores, num_proposals])
      import pdb
      pdb.set_trace()
  ...
  def _format_groundtruth_data(self, image_shape):
    ...
    Returns:
      ...
      groundtruth_classes_with_background_list: A list of 2-D one-hot
        ...
        class targets with the 0th index assumed to map to the background class.
    """
    gt_boxes = self.groundtruth_lists(fields.BoxListFields.boxes)
    gt_oriented_boxes = self.groundtruth_lists(fields.BoxListFields.oriented_boxes)

    groundtruth_boxlists = [
        box_list_ops.to_absolute_coordinates(
            box_list.BoxList(boxes, gt_oriented_boxes[i]), image_shape[1], image_shape[2])
  ...
  def _sample_box_classifier_minibatch(self,
    ...
    """
    (cls_targets, cls_weights, _, _, _, _) = self._detector_target_assigner.assign(
  ...
  def _postprocess_box_classifier(self,
                                  ...
                                  refined_box_encodings_oriented,
                                  class_predictions_with_background,
                                  proposal_boxes,
                                  num_proposals,
                                  image_shape,
    ...
    Args:
      ...
      refined_box_encodings_oriented: a 3-D tensor with shape
        [total_num_padded_proposals, num_classes, 8] representing predicted
        (final) refined box encodings.
    ...
    refined_box_encodings_batch = tf.reshape(refined_box_encodings,
                                             [-1, self.max_num_proposals,
                                              self.num_classes,
                                              self._detector_box_coder.code_size])
    refined_box_encodings_batch_oriented = tf.reshape(refined_box_encodings_oriented,
            [-1, self.max_num_proposals,
                self.num_classes, self._detector_box_coder.code_size_oriented])
    ...
    refined_decoded_oriented_boxes_batch = self._batch_decode_oriented_boxes(
        refined_box_encodings_batch_oriented, proposal_boxes)
#    refined_decoded_oriented_boxes_batch = tf.reshape(refined_decoded_oriented_boxes_batch,
    ...
    #simply using the original nms for now
    (nmsed_boxes, nmsed_oriented_boxes, nmsed_scores, nmsed_classes,
     nmsed_masks, num_detections) = self._second_stage_nms_fn(
         refined_decoded_boxes_batch,
         refined_decoded_oriented_boxes_batch,
    ...
    detections = {'detection_boxes': nmsed_boxes,
                  'detection_oriented_boxes': nmsed_oriented_boxes,
  ...
  def _batch_decode_boxes(self, box_encodings, anchor_boxes):
    ...
    decoded_boxes = self._proposal_box_coder.decode(
        tf.reshape(box_encodings, [-1, self._proposal_box_coder.code_size]),
        tiled_anchors_boxlist)
    return tf.reshape(decoded_boxes.get(),
                      tf.stack([combined_shape[0], combined_shape[1],
                                num_classes, 4]))

  def _batch_decode_oriented_boxes(self, oriented_box_encodings, anchor_boxes):
    """Decode tensor of refined oriented box encodings.

    Args:
      oriented_box_encodings: a 4-D tensor with shape
        [batch_size, max_num_proposals, num_classes, self._detector_box_coder.code_size]
        representing predicted (final) refined box encodings.
      proposal_boxes: [batch_size, self.max_num_proposals, 4] representing
        decoded proposal bounding boxes.
    ...
    Args:
      box_encodings: a 4-D tensor with shape
        [batch_size, num_anchors, num_classes, self._detector_box_coder.code_size]
        representing box encodings.
      anchor_boxes: [batch_size, num_anchors, 4] representing
        decoded bounding boxes.
    ...
    tiled_anchors_boxlist = box_list.BoxList(
        tf.reshape(tiled_anchor_boxes, [-1, 4]))
    decoded_boxes = self._detector_box_coder.decode_oriented(
        tf.reshape(oriented_box_encodings, [-1, self._detector_box_coder.code_size_oriented]),
        tiled_anchors_boxlist)
    return tf.reshape(decoded_boxes.get_oriented(),
                      tf.stack([combined_shape[0], combined_shape[1],
                                num_classes, 4, 2]))
  ...
  def loss(self, prediction_dict, scope=None):
    ...
    with tf.name_scope(scope, 'Loss', prediction_dict.values()):
      ...
      #visualize_box_list(None, groundtruth_boxlists[0], groundtruth_classes_with_background_list[0], normalized=False)
  ...
  def _loss_box_classifier(self,
    ...
    Args:
      ...
      refined_box_encodings_oriented: a 3-D tensor with shape
        [total_num_proposals, num_classes, box_coder.code_size_oriented] representing
        predicted (final) refined oriented box encodings.
    ...
    with tf.name_scope('BoxClassifierLoss'):
      ...
      (batch_cls_targets_with_background, batch_cls_weights, batch_reg_targets,
       batch_reg_weights, batch_reg_targets_oriented, batch_reg_weights_oriented,_) = target_assigner.detector_batch_assign_targets(
      ...
      refined_box_encodings_oriented_with_background = tf.pad(
          refined_box_encodings_oriented, [[0, 0], [1, 0], [0, 0]])
      ...
      #oriented boxes
      refined_box_encodings_masked_by_class_targets_oriented = tf.boolean_mask(
          refined_box_encodings_oriented_with_background,
          tf.greater(flat_cls_targets_with_background, 0))
      reshaped_refined_box_encodings_oriented = tf.reshape(
          refined_box_encodings_masked_by_class_targets_oriented,
          [batch_size, -1, 8])

      second_stage_loc_losses = self._second_stage_localization_loss(
          reshaped_refined_box_encodings,
          batch_reg_targets, weights=batch_reg_weights) / normalizer
      second_stage_loc_losses_oriented = self._second_stage_localization_loss_oriented(
          reshaped_refined_box_encodings_oriented,
          batch_reg_targets_oriented, weights=batch_reg_weights_oriented) / normalizer
      ...
      if False:
        ...
        with tf.Session(config=config) as sess:
          ...
          tf.train.start_queue_runners(sess)
          while(True):
            results = sess.run([second_stage_loc_losses,
                                second_stage_loc_losses_oriented,
                                reshaped_refined_box_encodings,
                                reshaped_refined_box_encodings_oriented,
                                batch_reg_targets,
                                batch_reg_targets_oriented,
                                batch_reg_weights,
                                batch_reg_weights_oriented])
            import pdb
            pdb.set_trace()
            print('a')
      ...
      second_stage_loc_loss_oriented = tf.reduce_sum(
          tf.boolean_mask(second_stage_loc_losses_oriented, paddings_indicator))
      ...
      loss_dict = {
          ...
          'second_stage_localization_loss_oriented':
          (self._second_stage_loc_loss_weight_oriented * second_stage_loc_loss_oriented),
