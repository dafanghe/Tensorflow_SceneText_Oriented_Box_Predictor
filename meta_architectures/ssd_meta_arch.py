...
class SSDMetaArch(model.DetectionModel):
  ...
  def __init__(self,
               ...
               oriented_localization_loss_weight,
    ...
    self._target_assigner = target_assigner.R2CNNTargetAssigner(
    ...
    self._oriented_localization_loss_weight = oriented_localization_loss_weight
  ...
  def predict(self, preprocessed_inputs):
    ...
    (box_encodings, oriented_box_encodings, class_predictions_with_background
    ) = self._add_box_predictions_to_feature_maps(feature_maps)
    predictions_dict = {
        'box_encodings': box_encodings,
        'oriented_box_encodings': oriented_box_encodings,
  ...
  def _add_box_predictions_to_feature_maps(self, feature_maps):
    ...
    for idx, (feature_map, num_anchors_per_location
      ...
      oriented_box_encodings = box_predictions[bpredictor.BOX_ENCODINGS_ORIENTED]
      ...
      oriented_box_encodings_shape = oriented_box_encodings.get_shape().as_list()
      if len(oriented_box_encodings_shape) != 5 or oriented_box_encodings_shape[2] != 1:
        raise RuntimeError('inclined_box_encodings from the box_predictor must be of '
                           'shape `[batch_size, num_anchors, 1, code_size]`; '
                           'actual shape', oriented_box_encodings_shape)

      box_encodings = tf.squeeze(box_encodings, axis=2)
      box_encodings_list.append(box_encodings)
      oriented_box_encodings = tf.squeeze(oriented_box_encodings, axis=2)
      oriented_box_encodings_list.append(oriented_box_encodings)
    ...
    with tf.control_dependencies([anchors_assert]):
      ...
      oriented_box_encodings = tf.concat(oriented_box_encodings_list, 1)
      class_predictions_with_background = tf.concat(
          cls_predictions_with_background_list, 1)
    return box_encodings, oriented_box_encodings, class_predictions_with_background
  ...
  def postprocess(self, prediction_dict):
    ...
    with tf.name_scope('Postprocessor'):
      ...
      oriented_box_encodings = prediction_dict['oriented_box_encodings']

      #decode regular bounding boxes
      detection_boxes = self._batch_decode(box_encodings)
      detection_boxes = tf.expand_dims(detection_boxes, axis=2)
      #decode oriented bounding boxes
      detection_oriented_boxes = self._batch_decode_oriented(oriented_box_encodings)
      detection_oriented_boxes = tf.expand_dims(detection_oriented_boxes, axis=2)
      ...
      (nmsed_boxes, nmsed_oriented_boxes, nmsed_scores, nmsed_classes, _,
       num_detections) = self._non_max_suppression_fn(detection_boxes,
                                                      detection_oriented_boxes,
                                                      detection_scores,
  ...
  def loss(self, prediction_dict, scope=None):
    ...
    with tf.name_scope(scope, 'Loss', prediction_dict.values()):
      (batch_cls_targets, batch_cls_weights, batch_reg_targets,
       batch_reg_weights, batch_reg_targets_oriented,
       batch_reg_weights_oriented, match_list) = self._assign_targets(
           self.groundtruth_lists(fields.BoxListFields.boxes),
           self.groundtruth_lists(fields.BoxListFields.oriented_boxes),
      ...
      batch_size = prediction_dict['oriented_box_encodings'].shape.as_list()[0]
      num_box = prediction_dict['oriented_box_encodings'].shape.as_list()[1]
      flattened_tensor = tf.reshape(prediction_dict['oriented_box_encodings'], [batch_size, num_box, 8])
      location_losses_oriented = self._localization_loss(
          flattened_tensor,
          batch_reg_targets_oriented,
          weights=batch_reg_weights_oriented)
      ...
      localization_loss_oriented = tf.reduce_sum(location_losses_oriented)
      ...
      loss_dict = {
          'localization_loss_oriented': (self._oriented_localization_loss_weight / normalizer) *
                               localization_loss_oriented,
  ...
  def _assign_targets(self, groundtruth_boxes_list,
          groundtruth_oriented_boxes_list, groundtruth_classes_list):
    ...
    Args:
      ...
      groundtruth_oriented_boxes_list: a list of 2-D tensors of shape [num_boxes, 4, 2]
        containing coordinates of the groundtruth boxes.
          format and assumed to be normalized and clipped
          relative to the image window with y_min <= y_max and x_min <= x_max.
    ...
    groundtruth_boxlists = [
        box_list.BoxList(boxes, oriented_boxes) for (boxes, oriented_boxes) in zip(groundtruth_boxes_list, groundtruth_oriented_boxes_list)
    ]
    groundtruth_classes_with_background_list = [
        tf.pad(one_hot_encoding, [[0, 0], [1, 0]], mode='CONSTANT')
        for one_hot_encoding in groundtruth_classes_list
    ]
    return target_assigner.detector_batch_assign_targets(
  ...
  def _batch_decode_oriented(self, oriented_box_encodings):
    ...
    decoded_oriented_boxes = self._box_coder.decode_oriented(
        tf.reshape(oriented_box_encodings, [-1, self._box_coder.code_size_oriented]),
        tiled_anchors_boxlist)
    return tf.reshape(decoded_oriented_boxes.get_oriented(),
