...
def scale(boxlist, y_scale, x_scale, scope=None):
  ...
  with tf.name_scope(scope, 'Scale'):
    ...
    #scale the oriented boxes
    scaled_oriented_boxes = None
    if 'oriented_boxes' in boxlist.data:
      y, x = tf.split(value=boxlist.get_field('oriented_boxes'),
          num_or_size_splits=2, axis=2)
      y = y_scale * y
      x = x_scale * x
      scaled_oriented_boxes = tf.concat([y, x], 2)
    scaled_boxlist = box_list.BoxList(scaled_boxes,
        scaled_oriented_boxes)
    return _copy_extra_fields(scaled_boxlist, boxlist)


def scale_oriented_box(oriented_box, y_scale, x_scale, scope=None):
  with tf.name_scope(scope, 'Scale_Oriented'):
    y_scale = tf.cast(y_scale, tf.float32)
    x_scale = tf.cast(x_scale, tf.float32)
    y, x = tf.split(
        value=oriented_box, num_or_size_splits=2, axis=2)
    new_y = y_scale * y
    new_x = x_scale * x
    new_oriented_box = tf.concat([new_y, new_x], 2)
    return new_oriented_box
...
def clip_to_window(boxlist, window, filter_nonoverlapping=True, scope=None):
  ...
  with tf.name_scope(scope, 'ClipToWindow'):
    ...
    clipped_inclined_boxes = None
    if boxlist.has_field('oriented_boxes'):
      y, x = tf.split(value=boxlist.get_oriented(), num_or_size_splits=2, axis=2)
      y_clipped = tf.maximum(tf.minimum(y, win_y_max), win_y_min)
      x_clipped = tf.maximum(tf.minimum(x, win_x_max), win_x_min)
      clipped_inclined_boxes = tf.concat([y_clipped, x_clipped], 2)
    clipped_boxes = tf.concat([y_min_clipped, x_min_clipped, y_max_clipped, x_max_clipped], 1)
    clipped = box_list.BoxList(clipped_boxes, clipped_inclined_boxes)
...
def change_coordinate_frame(boxlist, window, scope=None):
  ...
  with tf.name_scope(scope, 'ChangeCoordinateFrame'):
    ...
    #cut the oriented box
    if boxlist.has_field('oriented_boxes'):
        cur_oriented_box = boxlist.get_field('oriented_boxes')
        cur_oriented_box = cur_oriented_box - [window[0], window[1]]
        cur_oriented_box = scale_oriented_box(cur_oriented_box,
            1.0 / win_height, 1.0 / win_width)
        boxlist_new.add_field('oriented_boxes', cur_oriented_box)
  ...
  with tf.name_scope(scope, 'Gather'):
    ...
    if boxlist.has_field('oriented_boxes'):
      subboxlist = box_list.BoxList(tf.gather(boxlist.get(), indices),
        tf.gather(boxlist.get_oriented(), indices))
    else:
      subboxlist = box_list.BoxList(tf.gather(boxlist.get(), indices))
    if fields is None:
      fields = boxlist.get_extra_fields()
...
def concatenate(boxlists, fields=None, scope=None):
  ...
  with tf.name_scope(scope, 'Concatenate'):
    ...
    if boxlist.has_field('boxes'):
      concat_boxlist = tf.concat([boxlist.get() for boxlist in boxlists], 0)
    concat_oriented_boxlist = None
    if boxlist.has_field('oriented_boxes'):
      concat_oriented_boxlist = tf.concat([boxlist.get_oriented() for boxlist in boxlists], 0)
    concatenated = box_list.BoxList(boxes=concat_boxlist,
        oriented_boxes=concat_oriented_boxlist)
    if fields is None:
...
def pad_or_clip_box_list(boxlist, num_boxes, scope=None):
  ...
  with tf.name_scope(scope, 'PadOrClipBoxList'):
    ...
    if boxlist.has_field('boxes'):
        processed_boxes = shape_utils.pad_or_clip_tensor(boxlist.get(),
            num_boxes)
