...
def simple_multiclass_non_max_suppression(boxes,
                                          oriented_boxes,
                                          scores,
                                          score_thresh,
                                          iou_thresh,
  ...
  Args:
    boxes: A [k, q, 4] float32 tensor containing k detections. `q` can be either
      number of classes or 1 depending on whether a separate box is predicted
      per class.
    oriented_boxes: A [k, q, 8] float32
    scores: A [k, num_classes] float32 tensor containing the scores for each of
      the k detections.
    score_thresh: scalar threshold for score (low scoring boxes are removed).
    iou_thresh: scalar threshold for IOU (new boxes that have high IOU overlap
  ...
  with tf.name_scope(scope, 'MultiClassNonMaxSuppression'):
    ...
    selected_boxes_list = []
    per_class_boxes_list = tf.unstack(boxes, axis=1)
    per_class_oriented_boxes_list = tf.unstack(oriented_boxes, axis=1)
    if masks is not None:
      per_class_masks_list = tf.unstack(masks, axis=1)
    boxes_ids = (range(num_classes) if len(per_class_boxes_list) > 1
                 else [0] * num_classes)
    for class_idx, boxes_idx in zip(range(num_classes), boxes_ids):
      per_class_boxes = per_class_boxes_list[boxes_idx]
      per_class_oriented_boxes = per_class_oriented_boxes_list[boxes_idx]
      boxlist_and_class_scores = box_list.BoxList(per_class_boxes,
              per_class_oriented_boxes)
      with tf.control_dependencies([length_assert]):
        class_scores = tf.reshape(
            tf.slice(scores, [0, class_idx], tf.stack([num_scores, 1])), [-1])
      boxlist_and_class_scores.add_field(fields.BoxListFields.scores,
...
def simple_batch_multiclass_non_max_suppression(
                                         boxes,
                                         oriented_boxes,
                                         scores,
                                         score_thresh,
  ...
  with tf.name_scope(scope, 'BatchMultiClassNonMaxSuppression'):
    ...
    def single_image_nms_fn(args):
      """Runs NMS on a single image and returns padded output."""
      (per_image_boxes, per_image_oriented_boxes, per_image_scores,
       per_image_masks, per_image_num_valid_boxes) = args
      per_image_boxes = tf.reshape(
          tf.slice(per_image_boxes, 3 * [0],
                   tf.stack([per_image_num_valid_boxes, -1, -1])), [-1, q, 4])
      per_image_oriented_boxes = tf.reshape(
          tf.slice(per_image_oriented_boxes, 4 * [0],
                   tf.stack([per_image_num_valid_boxes, -1, -1, -1])), [-1, q, 4, 2])
      per_image_scores = tf.reshape(
          tf.slice(per_image_scores, [0, 0],
                   tf.stack([per_image_num_valid_boxes, -1])),
          [-1, num_classes])
      ...
      per_image_masks = tf.reshape(
          ...
          [-1, q, per_image_masks.shape[2].value,
           per_image_masks.shape[3].value])
      nmsed_boxlist = simple_multiclass_non_max_suppression(
          per_image_boxes,
          per_image_oriented_boxes,
          per_image_scores,
          score_thresh,
          iou_thresh,
          max_size_per_class,
      ...
      num_detections = nmsed_boxlist.num_boxes()
      nmsed_boxes = padded_boxlist.get()
      nmsed_oriented_boxes = padded_boxlist.get_field('oriented_boxes')
      nmsed_scores = padded_boxlist.get_field(fields.BoxListFields.scores)
      nmsed_classes = padded_boxlist.get_field(fields.BoxListFields.classes)
      nmsed_masks = padded_boxlist.get_field(fields.BoxListFields.masks)
      return [nmsed_boxes, nmsed_oriented_boxes, nmsed_scores, nmsed_classes,
              nmsed_masks, num_detections]

    (batch_nmsed_boxes, batch_nmsed_oriented_boxes,
     batch_nmsed_scores, batch_nmsed_classes, batch_nmsed_masks,
     batch_num_detections) = tf.map_fn(
         single_image_nms_fn,
         elems=[boxes, oriented_boxes, scores, masks, num_valid_boxes],
         dtype=[tf.float32, tf.float32, tf.float32, tf.float32, tf.float32, tf.int32],
         parallel_iterations=parallel_iterations)

    if original_masks is None:
      batch_nmsed_masks = None
    return (batch_nmsed_boxes, batch_nmsed_oriented_boxes,
            batch_nmsed_scores, batch_nmsed_classes,
            batch_nmsed_masks, batch_num_detections)
