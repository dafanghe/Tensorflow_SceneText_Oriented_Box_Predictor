...
def visualize_box_list(image, boxlist):
  ...
  if not isinstance(image, tf.Tensor):
      image = tf.convert_to_tensor(np.zeros((500, 500, 3), np.uint8))
  test_num = 5
...
def flip_oriented_boxes(oriented_boxes):
  """Left-right flip the oriented boxes.

  Args:
    oriented_boxes: rank 3 float32 tensor containing the bounding boxes -> [N, 4, 2].
           Boxes are in normalized form meaning their coordinates vary
           between [0, 1].

  Returns:
    Flipped oriented_boxes.
  """
  # Flip boxes.
  #y1, x1, y2, x2, y3, x3, y4, x4 = tf.split(value=oriented_boxes, num_or_size_splits=8,
  #        axis=2)
  y, x = tf.split(value=oriented_boxes, num_or_size_splits=2, axis=2)
  flipped_x = tf.subtract(1.0, x)
  flipped_oriented_boxes = tf.concat([y, flipped_x], 2)
  return flipped_oriented_boxes
...
def random_horizontal_flip(
  ...
  Args:
    ...
    boxes: (optional) rank 2 float32 tensor with shape [N, 4]
           ...
           between [0, 1].
           Each row is in the form of [ymin, xmin, ymax, xmax].
    oriented_boxes: (optional) rank 3, float 32 with shape [N, 4, 2]
            containing the oriented boxes for each object.
  ...
  Returns:
    ...
    boxes: rank 2 float32 tensor containing the bounding boxes -> [N, 4].
           Boxes are in normalized form meaning their coordinates vary
           between [0, 1].
    oriented_boxes: rank 3 float32 tensor containing the oriented boxes
  ...
  with tf.name_scope('RandomHorizontalFlip', values=[image, boxes]):
    ...
    # flip oriented boxes
    if oriented_boxes is not None:
      oriented_boxes = tf.cond(
          do_a_flip_random, lambda: flip_oriented_boxes(oriented_boxes), lambda: oriented_boxes)
      result.append(oriented_boxes)
...
def _strict_random_crop_image(image,
  ...
  with tf.name_scope('RandomCropImage', values=[image, boxes]):
    ...
    boxlist.add_field('oriented_boxes', oriented_boxes)
    ...
    new_oriented_box = new_boxlist.get_field('oriented_boxes')
    new_oriented_box = tf.clip_by_value(
            new_oriented_box, clip_value_min=0.0, clip_value_max=1.0)
    result = [new_image, new_boxes, new_oriented_box, new_labels]

    #visualize_box_list(new_image, new_boxlist)
    if masks is not None:
...
def rotate_point(point, angle, center):
    new_x = (point[0] - center[0]) * math.cos(angle) - (point[1] - center[1]) * math.sin(angle) + center[0]
    new_y = (point[0] - center[0]) * math.sin(angle) + (point[1] - center[1]) * math.cos(angle) + center[1]
    return (new_x, new_y)


def random_rotate_image(image_tf, boxes_tf, oriented_boxes_tf, min_angle=-30, max_angle=30):
  """Randomly rotate the image and boxes.
  """
  def wrap_func(image_np, oriented_boxes_np, min_angle, max_angle):
      """
      Args:
        image_np: (?, ?, 3) float32
        oriented_boxes_np: (?, 4, 2) float32
      """
      if min_angle > max_angle:
          return image_np, oriented_boxes_np
      height, width, dim = image_np.shape
      oriented_boxes_np[:, :, 0] = oriented_boxes_np[:, :, 0] * height
      oriented_boxes_np[:, :, 1] = oriented_boxes_np[:, :, 1] * width

      angle = random.randint(min_angle, max_angle)
      center = (width/2, height/2)
      max_size = max(height, width)
      new_patch = np.zeros((2 * max_size + height, 2 * max_size+width, 3), np.float32)
      new_patch[max_size:max_size+height, max_size:max_size+width, :] = image_np
      new_height, new_width, dim = new_patch.shape
      new_center = (max_size + width/2, max_size + height/2)
      #rotate
      M = cv2.getRotationMatrix2D(new_center, angle, 1.0)
      rotated = cv2.warpAffine(new_patch, M, (new_width * 2, new_height * 2))
      #crop
      indices = np.where(rotated > 0)
      minx = min(indices[1])
      miny = min(indices[0])
      maxx = max(indices[1])
      maxy = max(indices[0])
      rotated = rotated[miny:maxy, minx:maxx, :]
      minx_translate = max_size
      miny_translate = max_size
      for i in range(0, oriented_boxes_np.shape[0]):
          points = [(oriented_boxes_np[i][j][1], oriented_boxes_np[i][j][0]) for j in range(4)]
          new_points = np.zeros((4, 2), np.float32)
          for j, point in enumerate(points):
              new_point = rotate_point(point, -angle * math.pi/180, center)
              new_point = (new_point[0] + minx_translate - minx,
                      new_point[1] + miny_translate - miny)
              new_points[j, 0] = new_point[1] #y
              new_points[j, 1] = new_point[0] #x
          oriented_boxes_np[i][:] = new_points
      oriented_boxes_np[:, :, 0] = oriented_boxes_np[:, :, 0] / rotated.shape[0]
      oriented_boxes_np[:, :, 1] = oriented_boxes_np[:, :, 1] / rotated.shape[1]
      return rotated, oriented_boxes_np
  image, oriented_boxes = tf.py_func(wrap_func, [image_tf, oriented_boxes_tf, min_angle, max_angle],
          [tf.float32, tf.float32])

  image.set_shape([None, None, 3])
  oriented_boxes.set_shape([None, 4, 2])
  minx = tf.reduce_min(oriented_boxes[:, :, 1], axis=1, keep_dims=True)
  miny = tf.reduce_min(oriented_boxes[:, :, 0], axis=1, keep_dims=True)
  maxx = tf.reduce_max(oriented_boxes[:, :, 1], axis=1, keep_dims=True)
  maxy = tf.reduce_max(oriented_boxes[:, :, 0], axis=1, keep_dims=True)
  boxes = tf.concat([miny, minx, maxy, maxx], axis=1)
  #clip value
  boxes = tf.clip_by_value(boxes, clip_value_min=0.0, clip_value_max=1.0)
  oriented_boxes = tf.clip_by_value(oriented_boxes, clip_value_min=0.0, clip_value_max=1.0)
  return (image, boxes, oriented_boxes)
...
def random_crop_image(image,
  ...
  if random_coef < sys.float_info.min:
    ...
    outputs = [image, boxes, oriented_boxes, labels]
    if masks is not None:
...
def random_crop_pad_image(image,
  ...
  cropped_image, cropped_boxes, oriented_boxes, cropped_labels = random_crop_image(
      image=image,
      boxes=boxes,
...
def ssd_random_crop(image,
  ...
  Args:
    ...
    boxes: rank 2 float32 tensor containing the bounding boxes -> [N, 4].
           ...
           between [0, 1].
           Each row is in the form of [ymin, xmin, ymax, xmax].
    oriented_boxes -> rank 3 float32 tensor containing
           oriented boxes -> (N, 4, 2)
  ...
  Returns:
    ...
    boxes: boxes which is the same rank as input boxes.
           Boxes are in normalized form.
    oriented_boxes: oriented_boxes which is the same rank as input
           oriented boxes.
    labels: new labels.
  ...
  def random_crop_selector(selected_result, index):
    ...
    """
    i = 4
    image, boxes, oriented_boxes, labels = selected_result[:i]
    selected_masks = None
  ...
  result = _apply_with_random_selector_tuples(
      tuple(
          t for t in (image, boxes, oriented_boxes, labels, masks, keypoints) if t is not None),
...
def ssd_random_crop_pad(image,
  ...
  def random_crop_pad_selector(image_boxes_labels, index):
    image, boxes, oriented_boxes, labels = image_boxes_labels
  ...
  new_image, new_boxes, new_oriented_boxes, new_labels = _apply_with_random_selector_tuples(
      (image, boxes, oriented_boxes, labels),
      random_crop_pad_selector,
      num_cases=len(min_object_covered))
  return new_image, new_boxes, new_oriented_boxes, new_labels
...
def ssd_random_crop_fixed_aspect_ratio(
  ...
  crop_result = ssd_random_crop(image, boxes, oriented_boxes, labels, masks,
                                keypoints, min_object_covered, aspect_ratio_range,
...
def get_default_func_arg_map(include_instance_masks=False,
  ...
  prep_func_arg_map = {
      ...
      random_horizontal_flip: (fields.InputDataFields.image,
                               ...
                               fields.InputDataFields.groundtruth_oriented_boxes,
                               groundtruth_instance_masks,
      ...
      ssd_random_crop_fixed_aspect_ratio: (
          ...
          groundtruth_keypoints,),
    ##customized
      random_rotate_image: (fields.InputDataFields.image,
                            fields.InputDataFields.groundtruth_boxes,
                            fields.InputDataFields.groundtruth_oriented_boxes,)
  }
...
def visualize_tensor_dict(tensor_dict):
  ...
  with tf.Session(config=config) as sess:
    ...
    while True:
      ...
      im = results['image'][0].astype(np.uint8)
      boxes = results['groundtruth_boxes']
      oriented_boxes = results['oriented_boxes']
...
def preprocess(tensor_dict, preprocess_options, func_arg_map=None):
  ...
  if False:
    visualize_tensor_dict(tensor_dict)
