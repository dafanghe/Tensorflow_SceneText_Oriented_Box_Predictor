...
BOX_ENCODINGS_ORIENTED = 'box_encodings_oriented'
...
class MaskRCNNBoxPredictor(BoxPredictor):
  ...
  def __init__(self,
    ...
    self._box_code_size_oriented = box_code_size_oriented
  ...
  def _predict(self, image_features, num_predictions_per_location):
    ...
    with slim.arg_scope(self._fc_hyperparams):
      ...
      box_encodings_oriented = slim.fully_connected(
          flattened_image_features,
          self._num_classes * self._box_code_size_oriented,
          activation_fn=None,
    ...
    box_encodings_oriented = tf.reshape(
        box_encodings_oriented, [-1, 1, self._num_classes, self._box_code_size_oriented])
...
class OrientedConvolutionalBoxPredictor(BoxPredictor):
  """Oriented Convolutional Box Predictor.
  ...
  def __init__(self,
    ...
    Raises:
      ValueError: if min_depth > max_depth.
    """
    super(OrientedConvolutionalBoxPredictor, self).__init__(is_training, num_classes)
    if min_depth > max_depth:
      raise ValueError('min_depth should be less than or equal to max_depth')
    self._conv_hyperparams = conv_hyperparams
    self._min_depth = min_depth
    ...
    self._kernel_size = kernel_size
    self._box_code_size = box_code_size
    self._oriented_box_code_size = oriented_box_code_size
  ...
  def _predict(self, image_features, num_predictions_per_location):
    ...
    with slim.arg_scope(self._conv_hyperparams), \
      ...
      with slim.arg_scope([slim.conv2d], activation_fn=None,
        ...
        oriented_box_encodings = slim.conv2d(
            net, num_predictions_per_location * self._oriented_box_code_size,
            [self._kernel_size, self._kernel_size],
            scope='OrientedBoxEncodingPredictor')
        if self._use_dropout:
          net = slim.dropout(net, keep_prob=self._dropout_keep_prob)
    ...
    oriented_box_encodings = tf.reshape(
        oriented_box_encodings, tf.stack([combined_feature_map_shape[0],
                                 combined_feature_map_shape[1] *
                                 combined_feature_map_shape[2] *
                                 num_predictions_per_location,
                                 1, 4, 2]))
