...
class R2CNNTargetAssigner(object):
  """Target assigner to compute classification and regression targets."""
  ...
  def assign(self, anchors, groundtruth_boxes, groundtruth_labels=None,
    ...
    with tf.control_dependencies([shape_assert]):
      match_quality_matrix = self._similarity_calc.compare(groundtruth_boxes,
                                                           anchors)
      match = self._matcher.match(match_quality_matrix, **params)
      reg_targets, reg_targets_oriented = self._create_regression_targets(
      ...
      if num_anchors is not None:
        reg_targets = self._reset_target_shape(reg_targets, num_anchors)
        reg_targets_oriented = self._reset_target_shape(reg_targets_oriented, num_anchors)
        cls_targets = self._reset_target_shape(cls_targets, num_anchors)
        reg_weights = self._reset_target_shape(reg_weights, num_anchors)
        cls_weights = self._reset_target_shape(cls_weights, num_anchors)
    return cls_targets, cls_weights, reg_targets, reg_targets_oriented, reg_weights, match
  ...
  def _create_regression_targets(self, anchors, groundtruth_boxes, match):
    ...
    Returns:
      regular_reg_targets: a float32 tensor with shape [N, box_code_dimension]
      oriented_reg_targets: a float32 tensor with shape [N, box_code_dimension]
    """
    matched_anchor_indices = match.matched_column_indices()
    unmatched_ignored_anchor_indices = (match.
                                        unmatched_or_ignored_column_indices())
    ...
    #oriented targets
    matched_reg_targets_oriented = self._box_coder.encode_oriented(
            matched_gt_boxes, matched_anchors)
    unmatched_ignored_reg_targets_oriented = tf.tile(
        self._default_regression_target_oriented(),
        tf.stack([tf.size(unmatched_ignored_anchor_indices), 1]))
    reg_targets_oriented = tf.dynamic_stitch(
        [matched_anchor_indices, unmatched_ignored_anchor_indices],
        [matched_reg_targets_oriented, unmatched_ignored_reg_targets_oriented])
    # TODO: summarize the number of matches on average.
    return reg_targets, reg_targets_oriented
  ...
  def _default_regression_target_oriented(self):
    """Returns the default oriented target for anchors to regress to.
    ...
    Returns:
      default_target: a float32 tensor with shape [1, box_code_dimension]
    """
    return tf.constant([self._box_coder.code_size_oriented*[0]], tf.float32)
...
def create_target_assigner(reference, stage=None,
  ...
  Returns:
    TargetAssigner: desired target assigner, either regular TargetAssigner or R2CNNTargetAssigner.
  ...
  if reference == 'FasterRCNN' and stage == 'proposal':
  ...
  elif reference == 'R2CNN' and stage == 'detection':
    similarity_calc = sim_calc.IouSimilarity()
    ...
    box_coder = faster_rcnn_box_coder.FasterRcnnBoxCoder(
        scale_factors=[10.0, 10.0, 5.0, 5.0],
        scale_factors_oriented=[10.0, 10.0, 10.0, 10.0, 10.0, 10.0, 10.0, 10.0])
    target_assigner = R2CNNTargetAssigner(similarity_calc, matcher, box_coder,
                        positive_class_weight=positive_class_weight,
                        negative_class_weight=negative_class_weight,
                        unmatched_cls_target=unmatched_cls_target)
  else:
...
def detector_batch_assign_targets(target_assigner,
                         anchors_batch,
                         gt_box_batch,
  ...
  reg_targets_list = []
  reg_weights_list = []
  reg_targets_oriented_list = []
  reg_weights_oriented_list = []
  match_list = []
  for anchors, gt_boxes, gt_class_targets in zip(
      anchors_batch, gt_box_batch, gt_class_targets_batch):
    (cls_targets, cls_weights, reg_targets, reg_targets_oriented,
    ...
    reg_targets_oriented_list.append(reg_targets_oriented)
    reg_weights_oriented_list.append(reg_weights)
    match_list.append(match)
  batch_cls_targets = tf.stack(cls_targets_list)
  batch_cls_weights = tf.stack(cls_weights_list)
  batch_reg_targets = tf.stack(reg_targets_list)
  batch_reg_weights = tf.stack(reg_weights_list)
  batch_reg_targets_oriented = tf.stack(reg_targets_oriented_list)
  batch_reg_weights_oriented = tf.stack(reg_weights_oriented_list)
  return (batch_cls_targets, batch_cls_weights, batch_reg_targets,
          batch_reg_weights, batch_reg_targets_oriented,
          batch_reg_weights_oriented, match_list)
